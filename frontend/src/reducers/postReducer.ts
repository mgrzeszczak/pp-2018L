...
// import * as data from "../../thread.json";
const data: any = require("../../obama-questions.json");
export interface PostData {
    posts: Post[];
}

function createThread(): PostData {
    const threadData: any = data;

    console.log(threadData);
    let postId = 1;
    let posts = threadData.slice(0, 1000).map((p: any) => new Post(postId++, p.author, p.body));
    return {
        posts
    };
}
...
export function postReducer(state: PostData = createThread(), action: any): PostData {
    switch (action.type) {
